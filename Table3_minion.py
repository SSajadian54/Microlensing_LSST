...
dat1=np.loadtxt('./files/Tab3a_minionA.txt') 

num2=int(Nf)
dat2=np.zeros((num2,31))
dat2=np.loadtxt('./files/Tab3a_minionB.txt') 
...
for i in range(Nf):
    if(float(dat2[i,30])>200.0 and dat2[i,28]==10.0):# epoch=900.0, cadence =4.0 days ;  Tobs=10 years
        nevent_com[0]+=float(dat2[i,19])
        number[0] += 1.0; 
        ave2[:,0] += dat2[i,:]  
        aepi[0] += float(float(dat2[i,4])/float(dat2[i,3]))
        for j in range(fi):
            ave1[:,j,0]+=dat1[count,:]
            count+=1   
    elif(float(dat2[i,30])<200.0 and dat2[i,28]==1.0):#epoch =180.0,  cadence=0.9 days;  Tobs=1 year
        nevent_com[1]+= float(dat2[i,19])
        number[1]+= 1.0; 
        ave2[:,1] += dat2[i,:]
        aepi[1] += float(float(dat2[i,4])/float(dat2[i,3]))
        for j in range(fi):
            ave1[:,j,1] +=dat1[count,:]
            count+=1   
    else:
...
##############################################
print "count: ", count 
print "No. fields with <cadence>~ 4.0 days: ", number[0], "<cadence>: ", ave2[29,0]/number[0] 
print "No. fields with <cadence>~ 1.0 days: ", number[1], "<cadence>: ", ave2[29,1]/number[1]
print "<epsilon_LSST>[minion I ]: ",  aepi[0]/number[0], "<epsilon_LSST> (minion, II)", aepi[1]/number[1]
print "No. comulative events (I) ",  nevent_com[0],  "No. colulative events (II)", nevent_com[1]

fil=open("Table3_minion.dat","w")
fil.close()
sav=np.zeros((2))


for i in range(31):
    fil=open("Table3_minion.dat","a+")
