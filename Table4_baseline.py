...
data1=np.loadtxt('./files/Tab4b_baselineB.txt') 
data2=np.zeros((num*fi*2,19))
data2=np.loadtxt('./files/Tab4b_baselineA.txt')
...
################################################################################
    ...
    if(k==0):   
        ...
        fil1.close()
        if(data1[i,30]!=160 or data1[i,29]!=20.93827 or data1[i,28]!=10.0):
        ...
        fil2.close() 
        if(data1[i,30]!=900 or data1[i,29]!=3.79871 or data1[i,28]!=10.0):
            print "ERROR please check,   k", k,  "  epoch: ",  data1[i,30], "  cadence: ", data1[i,29], "  Tobs: ", data1[i,28] 
            input("Enter a number")
    else:
        print "ERROR k: ", k
...
fil1.close();    fil2.close()
fil4=open("Table4.dat","w");  fil5=open("Table4_baselineb.dat","w")
fil4.close();    fil5.close(); 
for i in range(num):
    k=int(counter[i])
    fil1=open("tab4A2.dat","a+") ;      fil2=open("tab4B2.dat","a+");  
    fil4=open("Table4.dat","a+")
    fil5=open("Table4_baselineb.dat","a+")
    ...
    last[9]=float(data[k,7]);last[10]=float(data2[k,7])### log10(gama)
    ...
    fil4.close()
    fil2.close();  fil1.close() ; fil5.close()
