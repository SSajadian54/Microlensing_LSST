...
data1=np.loadtxt('./files/Tab4a_minionB.txt') 
data2=np.zeros((num*fi*2,19))
data2=np.loadtxt('./files/Tab4a_minionA.txt')
ne_sum=np.zeros((2))
...
################################################################################
    ...
    if(k==0):   
        ...
        fil1.close()
        if(data1[i,30]!=900 or data1[i,29]!=3.93422 or data1[i,28]!=10.0):
        ...
        fil2.close() 
        if(data1[i,30]!=180 or data1[i,29]!=0.96963 or data1[i,28]!=1.0):
            print "ERROR please check,  k", k,  "  epoch: ",  data1[i,30], "  cadence: ", data1[i,29], "  Tobs: ", data1[i,28] 
            input("Enter a number")
    else:
        print "ERROR k: ", k
...
fil1.close();    fil2.close()
fil4=open("Table5.dat","w")
fil5=open("Table5_minionb.dat","w")
fil4.close();  fil5.close()
for i in range(num):
    k=int(counter[i])
    fil1=open("tab4A2.dat","a+") ;      fil2=open("tab4B2.dat","a+");  fil4=open("Table5.dat","a+")
    fil5=open("Table5_minionb.dat","a+")
    ...
    last[7]=float(data[k,6]); last[8]=float(data2[k,6])### blending
    last[9]=float(data[k,7]);last[10]=float(data2[k,7])### log10(Gama*10^7)
    last[11]=float(data[k,8]); last[12]=float(data2[k,8])### Ne(deg^-2)
    last[13]=float(data[k,9]); last[14]=float(data2[k,9])### log10(epsilon)
    ...
    if(float(last[4])>float(last[3])):
        #print "ERROR row: ", i, "log10(tE)[I]: ",  last[3], "log10(tE)[II]: ", last[4], "b(deg): ",  last[2]
        nte+=1
    if(float(last[12])>float(last[11])):
        print "ERROR row: ", i, "Ne(deg^-2)[I]: ",  last[3], "Ne(deg^-2)[II]: ", last[4], "b(deg): ",  last[2]
    if(float(last[14])<float(last[13])):
        print "ERROR row: ", i, "epcilon[I]: ",  last[13], "epcilon[II]: ", last[14], "b(deg): ",  last[2]
    if(float(last[10])<float(last[9])):
        print "ERROR row: ", i, "Gama[I]: ",   last[9], " Gama[II]: ", last[10],  "b(deg): ",  last[2]
...
print "No.  of events with brighter source star with startegy II: ", nmag
print "<epsilon[II]/epsilon[I]>: ", float(ave_rate/num)
