...
dat1=np.loadtxt('./files/Tab3b_baselineA.txt') 

num2=int(Nf)
dat2=np.zeros((num2,31))
dat2=np.loadtxt('./files/Tab3b_baselineB.txt') 
...
for i in range(Nf):
    if(float(dat2[i,30])<200.0 and dat2[i,28]==10.0):# epoch=200.0, cadence =20 days 
        ...
        for j in range(fi):
            ave1[:,j,0]+=dat1[count,:]
            count+=1   
    else:                    ### epoch =900.0,  cadence=4 days 
...
##############################################
print "count: ", count 
print "No. fields with <cadence>~ 20 days: ", number[0], "<cadence>: ", ave2[29,0]/number[0] 
print "No. fields with <cadence>~4.0 days: ", number[1], "<cadence>: ", ave2[29,1]/number[1]
print "<epsilon_LSST>[baseline I ]: ",  aepi[0]/number[0], "<epsilon_LSST> (baseline, II)", aepi[1]/number[1]
print "No. comulative events (I) ",  nevent_com[0],  "No. colulative events (II)", nevent_com[1]

fil=open("Table3_baseline.dat","w")
fil.close()
sav=np.zeros((2))


for i in range(31):
    fil=open("Table3_baseline.dat","a+")
